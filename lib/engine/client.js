...
client.make = function(options){
    ...
    new_client.listening_socket.on("message", function(data){
      ...
      if (typeof parsed_payload.console != "undefined") {
      ...
      } else if (typeof parsed_payload.error != "undefined") {

        task.emit("eval", parsed_payload.error, null);
        ...
        var response = new TaskResponse(parsed_payload.response);

        task.emit("eval", null, response);
