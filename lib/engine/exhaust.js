...
exhaust.create = function(config){
    var logging_gateway = exhaust.find_or_create_logging_gateway(config);

    var provide_defaults = require("./helpers/provide_defaults");
    var options = provide_defaults(config,{
	listening_endpoint: "ipc://exhaust-listener.ipc",
	publishing_endpoint: "ipc://exhaust-publisher.ipc"
    });
