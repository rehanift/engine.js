...
cylinder.make = function(config){
    ...
    //new_cylinder.execution_watcher.on("kill", function(task_id){
    //    new_cylinder.exhaust_socket.send(JSON.stringify({
    //        task_id: task_id,
    //        last_eval: "TimeoutError: Script took too long to execute"
    //    }));
    //});
	...
	new_cylinder.execution_watcher.start();
        console.log("cylinder", config.id, data.toString());
    });
