...
var cylinder = function(id, listening_socket, sending_socket, results_socket, exhaust_socket, 
                        execution_watcher, process_spawner, logging_gateway, context_validator) {
    var self = this;
    self.id = id;
...
cylinder.make = function(config){
	...
	if (new_cylinder.context_validator.validate(parsed_data.context) == false) {
            new_cylinder.exhaust_socket.send(JSON.stringify({
		task_id: parsed_data.task_id,
		last_eval: "SandboxError: There was a problem with your task's context"
            }));	    
	} else {	
...
cylinder.create = function(config){
    ...
    sending_socket.bind("ipc:///tmp/"+id+".ipc", function(err){
        if (err) throw err;
    });

    var results_socket = context.socket("pull");
    results_socket.bind("ipc:///tmp/results-"+id+".ipc", function(err){
        if (err) throw err;
    });
